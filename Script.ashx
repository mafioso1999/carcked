...
public class Script : SingletonAsyncHandler, ICacheDependencyCreator
{
	...
	public override async Task ProcessRequestAsync(HttpContext context)
	{
		...
		var fileDownloadBaseName = await WebResources.GetStringAsync("FileDownload.BaseName");
		...
		await context.Response.Output.WriteAsync(JavaScriptSerializer.SerializeCode(new
		{
			...
			context = new
			{
				...
				installerHandlerPath = WebConfigurationManager.GetHandlerPath(typeof(InstallerHandler)).ReplaceFirst("*", fileDownloadBaseName),
				launchHandlerPaths = Extensions.GetPluginTypes(typeof(LaunchHandler))
					.Where(it => !it.IsAbstract)
					.ToDictionary(
						it => it.Name.TrimEnd("Handler"),
						it => WebConfigurationManager.GetHandlerPath(it).ReplaceFirst("*", fileDownloadBaseName)
					),
